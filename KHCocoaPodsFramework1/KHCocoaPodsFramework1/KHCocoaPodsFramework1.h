//
//  KHCocoaPodsFramework1.h
//  KHCocoaPodsFramework1
//
//  Created by Kevin Hawkins on 12/30/15.
//  Copyright © 2015 KH CocoaPods Tests. All rights reserved.
//

#import <UIKit/UIKit.h>

//! Project version number for KHCocoaPodsFramework1.
FOUNDATION_EXPORT double KHCocoaPodsFramework1VersionNumber;

//! Project version string for KHCocoaPodsFramework1.
FOUNDATION_EXPORT const unsigned char KHCocoaPodsFramework1VersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <KHCocoaPodsFramework1/PublicHeader.h>

#import <KHCocoaPodsFramework1/Framework1PublicClass1.h>
